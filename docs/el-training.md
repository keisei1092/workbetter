...
- [x] [rbenv](https://github.com/rbenv/rbenv)を利用して最新バージョンのRubyをインストールしてください
- [x] `ruby -v` コマンドでRubyのバージョンが表示されることを確認してください

#### 1-2: Railsのインストール

- [x] GemコマンドでRailsをインストールしましょう
- [x] 最新バージョンのRailsをインストールしてください
- [x] `rails -v` コマンドでRailsのバージョンが表示されることを確認してください

#### 1-3: データベース（PostgreSQL）のインストール

- [x] 手元のOSでPostgreSQLをインストールしましょう
  - macOSの場合は `brew` などでインストールしてください

### ステップ2: GitHubにリポジトリを作成しよう

- [x] 手元にGitをインストールしましょう
  - macOSの場合は `brew` などでインストールしてください
  - `gitconfig` でユーザ名、メールアドレスを登録しましょう
- [x] アプリ名（=リポジトリ名）を考えましょう
- [x] リポジトリを作成しましょう
  - アカウントがない場合は取得します
  - その上で空のリポジトリを作成します
...
- [x] `rails new` コマンドでアプリケーションに最低限必要なディレクトリやファイルを作成しましょう
- [x] `rails new` してできたプロジェクトのディレクトリ（アプリ名のディレクトリ）の直下に `docs` というディレクトリを作り、この文書ファイルをコミットしましょう
  - このアプリの仕様を管理下に置き、いつでも見られるようにするためです
- [x] 作成したアプリをGitHub上に作成したリポジトリにpushしましょう
- [x] バージョンを明示するため、利用するRubyのバージョンを `Gemfile` に記載しましょう（Railsは既にバージョンが記載されていることを確認しましょう）
...
- [x] `Gemfile` で `pg` （PostgreSQLのデータベースドライバ）をインストールしましょう
- [x] `database.yml` の設定をしましょう
- [x] `rails db:create` コマンドでデータベースの作成をしましょう
- [x] `rails db` コマンドでデータベースへの接続確認をしましょう
...
- タスクの一覧画面、作成画面、詳細画面、編集画面を作成しましょう
  - [x] `rails generate` コマンドでコントローラとビューを作成します
    - どのテンプレートエンジンを採用するかはメンターと相談して決めましょう
  - [x] コントローラとビューに必要な実装を追加しましょう
  - [x] 作成、更新、削除後はそれぞれflashメッセージを画面に表示させましょう
- [x] `routes.rb` を編集して、 `http://localhost:3000/` でタスクの一覧画面が表示されるようにしましょう
- GitHub上でPRを作成してレビューしてもらいましょう
  - 今後、PRが大きくなりそうだったらPRを2回以上に分けることを検討しましょう

### ステップ8: E2Eテストを書こう

- [x] specを書くための準備をしましょう
  - [x] `spec/spec_helper.rb` 、 `spec/rails_helper.rb` を用意しましょう
...
- [x] Railsのi18nの仕組みを利用して、日本語リソースの共通化をしましょう
...
- [x] 現在IDの順で並んでいますが、これを作成日時の降順で並び替えてみましょう
- [x] 並び替えがうまく行っていることをfeature specで書いてみましょう
...
- [x] Herokuにデプロイを実施してみましょう
  - アカウントがなければ作成します
- [x] デプロイされたHeroku上のアプリを触ってみよう
...
- [x] デプロイの方法を `README.md` に記載しましょう
  - その際に、このアプリで使っているフレームワークのバージョン情報なども記載しておくとなおよいです

### ステップ14: 終了期限を追加しよう

- [x] タスクに対して、終了期限を登録できるようにしてみましょう
- [x] 一覧画面で、終了期限でソートできるようにしましょう
- [x] specを拡充しましょう
